...
gulp.task('browserify', function() {
  return gulp.src('build/js/app.js')
    ...
    .pipe(rename('bulit.js')) // 对gulp流中的文件使用rename进行重命名
...
gulp.task('watch', () => {
  ...
  connect.server({
    name: 'Dev App',
    root: ['build'],// root是运行项目的代码在哪里，运行项目的目录
    port: 3000,
...
gulp.task('uglify', function () {
  return gulp.src('./build/js/bulit.js')// 引入需要运行的文件
    .pipe(uglify()) // 压缩js： 1. 将变量名改为单个字母 2. 去除多余的空格和换行符 3. 移除注释
    .pipe(rename('dist.min.js'))
    .pipe(gulp.dest('./dist/js'))// 这里面只能写目录。不能写文件名
});
...
// 当任务名为default的时候，执行任务的时候可以省略掉default，只写gulp
